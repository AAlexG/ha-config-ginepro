...
def monthly_delta(cur, mid):
    # Delta mensile = ULTIMO sum del mese - ULTIMO sum del mese precedente.
    ...
    #
    # v17: non piu' MAX(sum) ma il sum della riga piu' recente del mese.
    # MAX vale solo se la catena dei sum e' sempre crescente. Dopo una
    # rettifica manuale in Impostazioni -> Strumenti -> Statistiche la
    # catena contiene un gradino negativo, e MAX restituisce il valore
    # PRIMA del gradino: il mese rettificato risultava ancora gonfiato e
    # quello successivo sottostimato. Settembre 2026 sui deumidificatori
    # mostrava 97,2 kWh invece di 22,1 per questo motivo.
    # La forma "GROUP BY ym con MAX(start_ts)" e' il caso speciale SQLite
    # in cui le colonne non aggregate provengono dalla riga che soddisfa
    # MAX: https://www.sqlite.org/lang_select.html#bare_columns_in_an_aggregate_query
    if not mid:
        return {}
    rows = cur.execute("""
        SELECT strftime('%Y-%m', datetime(start_ts,'unixepoch','localtime')) AS ym,
               MAX(start_ts) AS ultima,
               sum AS fine
        FROM statistics
    ...
    for ym, _ultima, fine in rows:
        ...
        # dal valore del contatore), quindi l'ultimo sum del primo mese e' gia'
